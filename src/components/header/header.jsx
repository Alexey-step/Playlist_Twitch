...
const Header = () => {
  ...
  return (
    <div className="page-header">
      <Search/>
      <Link to="/favorites" className="page-header__link">
        <svg className={`favorite__svg ${favorites.length > 0 ? `favorite__svg--active` : ``}`} width="24" height="22" viewBox="0 0 24 22" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        </svg>  Favorites: <span>{favorites.length}</span>
      </Link>
    </div>
  )
}
